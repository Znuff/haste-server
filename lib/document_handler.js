...
DocumentHandler.prototype.handleRaw = function(data, socket) {
  var _this = this;

  if ( _this.maxLength && data.length > _this.maxLength ) {
    winston.warn('document >maxLength', { maxLength: _this.maxLength });
    socket.end('document > maxLength(' + _this.maxLength + ')');
  } else {
    _this.chooseKey(function (key) {
      _this.store.set(key, data, function (res) {
        if (res) {
          winston.verbose('added document', { key: key });
          socket.end('https://pastie.dev/' + key + ' | https://pastie.dev/raw/' + key + '\n');
        } else {
          winston.verbose('error adding document');
          socket.end('error adding document');
        }
      });
    });
  }
